...
function hasRequiredNetflixLinks(htmlContent, textContent) {
  const content = (htmlContent || '') + ' ' + (textContent || '');
  
  // চেক করুন প্রথম লিংক আছে কিনা (verify link)
  const hasVerifyLink = /https:\/\/www\.netflix\.com\/account\/travel\/verify\?nftoken=/i.test(content);
...
function isHouseholdEmail(htmlContent, textContent) {
  // শুধুমাত্র লিংক দিয়ে ফিল্টার
  return hasRequiredNetflixLinks(htmlContent, textContent);
}
...
async function processContentWithLinks(html, text) {
  ...
  if (linkPlaceholders.length === 0) {
    if (cleanedContent) {
      segments.push({ type: "text", value: cleanedContent });
    }
    return segments;
  }
        ...
        imap.search(["ALL"], (err, results) => {
          ...
          fetch.once("end", async () => {
            try {
              ...
              // ট্রান্সলেশন ছাড়াই শুধু লিংক দিয়ে ফিল্টার
              ...
              const formattedEmail = {
                ...
                subject: householdEmail.subject || "Email",
