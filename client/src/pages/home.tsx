...
import { Search, Mail, Lock, CheckCircle, AlertCircle, Loader2 } from "lucide-react";
...
export default function Home() {
  ...
  // Mock mutation to simulate API call
  const searchMutation = useMutation({
    mutationFn: async (data: FormValues) => {
      // Simulate network delay
      await new Promise((resolve) => setTimeout(resolve, 2000));
      
      // Mock logic: randomly succeed or fail to find code
      const mockSuccess = true; // Force success for demo
      
      if (!mockSuccess) {
        throw new Error("No recent Household Code emails found for this user.");
      }

      return {
        subject: "Important: How to update your Netflix Household",
        receivedAt: new Date().toISOString(),
        bodySnippet: "Your temporary access code is 4829. This code expires in 15 minutes.",
        accessCode: "4829",
      ...
      >
              ...
              <Card className="border-l-4 border-l-green-500 bg-card/80 backdrop-blur overflow-hidden">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-4 rounded-md bg-secondary/30 border border-border font-mono text-sm leading-relaxed">
                      {result.bodySnippet}
                    </div>
                  </div>
        ...
        <div className="text-center pt-8 opacity-40 hover:opacity-100 transition-opacity">
          <p className="text-xs text-muted-foreground">
            Note: This is a mockup. Backend integration required for real IMAP access.
          </p>
        </div>
      </motion.div>
    </div>
